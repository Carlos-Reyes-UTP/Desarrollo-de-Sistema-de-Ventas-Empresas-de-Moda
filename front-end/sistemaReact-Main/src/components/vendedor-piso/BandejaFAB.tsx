...
export function BandejaFAB({ onClick }: Props) {
  ...
  return (
    <button
      ...
      className="fixed bottom-24 right-6 z-[150] flex items-center gap-2.5 rounded-[2rem] app-btn-primary px-5 py-3.5 shadow-[0_8px_30px_rgba(0,0,0,0.25)] transition-all hover:scale-105 active:scale-95 animate-fadeIn"
      style={{
    ...
    >
      ...
      {/* Badge */}
      <span className="flex h-6 min-w-[1.5rem] items-center justify-center rounded-full bg-[var(--app-surface)] px-1.5 text-[11px] font-black text-[var(--app-text)] shadow-sm border border-[var(--app-border)]">
        {total}
      </span>
    </button>
  );
