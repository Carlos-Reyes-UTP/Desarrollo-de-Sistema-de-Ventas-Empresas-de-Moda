...
const SidebarMenu = ({ vistaActual, cambiarVista, usuario, cerrarSesion }: SidebarMenuProps) => {
  ...
  return (
    <>
      ...
      {/* Botón menú escritorio: siempre montado */}
      <div
        className={`fixed left-4 top-4 z-[40] hidden transition-opacity duration-200 ease-out md:block ${isDrawerOpen ? "pointer-events-none opacity-0" : "opacity-100"
          }`}
        aria-hidden={isDrawerOpen}
      >
        <button
          type="button"
          onClick={openDrawer}
          className="group flex h-12 w-12 items-center justify-center rounded-xl border border-gray-100 bg-white shadow-[0_4px_12px_-2px_rgba(0,0,0,0.08)] transition-all duration-200 hover:bg-gray-50"
        >
      ...
      <Drawer
        ...
        className="z-[10000] border-r border-white/5 bg-[#0a0a0a] shadow-2xl"
        overlay={true}
        placement="left"
        size={300}
        overlayProps={{
          className:
            "fixed inset-0 z-[9998] bg-black/40 backdrop-blur-sm will-change-[opacity] pointer-events-auto " +
